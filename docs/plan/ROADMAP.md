...
| `ai-background-removal` | 스티커 스튜디오에 로컬 AI 배경 제거(@bunnio/rembg-web MIT, u2netp 모델) + AI 보정 도구(올가미로 영역 지정 후 복원/삭제) 추가 | 완료 (step0~2, report 있음. @imgly/background-removal은 AGPL이라 배제하고 MIT 라이브러리로 전환. 모델 파일은 public/models/u2netp.onnx로 자체 서빙) |
...
| 순서 | Phase | 내용 | 비고 |
|---|---|---|---|
| 1 | `youtube-embed` | 다이어리에 유튜브 링크를 넣으면 재생 가능한 썸네일 카드로 표시 | |
| 2 | `desktop-custom` | 바탕화면 아이콘/배경화면 사용자 커스터마이징 | |
| 3 | `movie-review-standalone` | 영화 리뷰를 diary와 분리된 독립 메뉴로 재구성, 템플릿/배경 커스텀, 동일 편집기 재사용 | 가장 큼. 기존 PRD의 "다이어리 탭 하위" 구조에서 "독립 메뉴"로 재정의 필요 |

후속 후보(순서 미정):
- `paint-shapes` — 그림판 도형 그리기(직선/사각형/원). free-drawing에서 범위 분리 (2026-07-14 합의).
- `social-embed` — 트위터/인스타/일반 웹페이지 링크 카드. `youtube-embed`에서 범위 분리, 유튜브만 우선 진행 후 별도 coplan (2026-07-19 합의).
- `html-export` — JSON export를 HTML export로 전환하는 아이디어. 기존 저장/불러오기 체계 전체를 건드리는 큰 변경이라 별도 coplan 필요 (2026-07-19 합의).
